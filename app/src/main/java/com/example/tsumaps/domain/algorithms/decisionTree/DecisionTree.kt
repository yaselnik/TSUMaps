...
class DecisionTree (
    ...
    data class DecisionResult(
        val result: String,
        val path: List<DecisionPathStep>
    )
    ...
    fun classifyWithPath(sample: Sample): DecisionResult {
        val steps = mutableListOf<DecisionPathStep>()
        var current = root

        while (true) {
            when (current) {
                is DecisionTreeNode.Leaf -> return DecisionResult(current.result, steps)
                is DecisionTreeNode.Node -> {
                    val value = extractAttribute(sample, current.attribute)
                    steps.add(
                        DecisionPathStep(
                            attribute = current.attribute,
                            value = value,
                            availableBranches = current.children.keys.sorted()
                        )
                    )
                    current = current.children[value]
                        ?: current.children.values.firstOrNull()
                        ?: return DecisionResult("Unknown", steps)
                }
            }
        }
    }
    ...
    fun prettyPrint(): String {
        fun rec(node: DecisionTreeNode, indent: String): String {
            return when (node) {
                is DecisionTreeNode.Leaf -> indent + "→ " + node.result
                is DecisionTreeNode.Node -> buildString {
                    append(indent).append(node.attribute).append("\n")
                    node.children.entries.sortedBy { it.key }.forEach { (value, child) ->
                        append(indent).append("  [").append(value).append("]\n")
                        append(rec(child, indent + "    ")).append("\n")
                    }
                }.trimEnd()
            }
        }
